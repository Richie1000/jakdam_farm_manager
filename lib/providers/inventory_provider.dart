...
class InventoryProvider with ChangeNotifier {
  ...
  void _fetchInventoryItems() {
    ...
    _firestore.collection('inventory').doc(userId).collection('items').snapshots().listen(
      (snapshot) {
  ...
  Future<void> addItem(InventoryItem item) async {
    if (userId == null) return;
    await _firestore.collection('inventory').doc(userId).collection('items').add(item.toMap());
    _fetchInventoryItems(); // Refresh items after adding
  }

  Future<void> updateItemQuantity(String id, int quantity) async {
    if (userId == null) return;
    await _firestore.collection('inventory').doc(userId).collection('items').doc(id).update({
      'quantity': quantity,
    });
  ...
  Future<void> deleteItem(String id) async {
    if (userId == null) return;
    await _firestore.collection('inventory').doc(userId).collection('items').doc(id).delete();
    _fetchInventoryItems(); // Refresh items after deleting
  }

  Future<void> updateItem(InventoryItem updatedItem) async {
    try {
      final itemRef = _firestore.collection('inventory').doc(userId).collection('items').doc(updatedItem.id);
