...
class _ProductionBudgetScreenState extends State<ProductionBudgetScreen> {
  ...
  final TextEditingController _numberOfFishesController = TextEditingController();
  ...
  String _lastCalculatedUnit = 'Feet'; // Store the unit used in the last calculation
  ...
  void _calculateBudget() {
    if (_formKey.currentState!.validate()) {
      ...
      _waterVolume = (numberOfFishes / 1000) * 12000; // Total water volume in liters

      // Variables for volume in cubic units
      double volumeInCubicFeet = _waterVolume / 28.3168; // Convert volume from liters to cubic feet
      double volumeInCubicMeters = _waterVolume / 1000;  // Convert volume from liters to cubic meters
      ...
      _totalFeedBags = (_totalFeedRequired / 15).ceil(); // Number of bags, 1 bag = 15 kg

      setState(() {
        _showDuration = true;
        _duration = _selectedFishType == 'Catfish' ? 6 : 7; // Update duration based on selected fish type
  ...
  void _clearFields() {
    ...
    setState(() {
      ...
      _totalFeedBags = 0;
      _showDuration = false;
      _duration = _selectedFishType == 'Catfish' ? 6 : 7; // Reset duration to default based on fish type
              ...
              DropdownButtonFormField<String>(
                ...
                onChanged: (value) {
                  setState(() {
                    ...
                    _duration = value == 'Catfish' ? 6 : 7; // Update duration based on selected fish type
                  });
                },
              ),
              SizedBox(height: 20),
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.red, // Change the color to differentiate the button
                      ),
                    ),
                  ),
                ],
